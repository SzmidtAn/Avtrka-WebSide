...
public class HttpServletMultipartRequest extends HttpServletRequestWrapper {
	...
	public String toHtmlString() {
		...
		sbReturn.append("<li>Ignore on Maxlength: " + isIgnoreOnMaxLength()
				+ "</li>");

		if (isMultipartRequest()) {
			sbReturn.append("<li>Max Content Memory Threshold: "
					+ getMaxContentLengthMemoryThreshold() + "</li>");

			sbReturn.append("<li>Load into memory: " + isLoadIntoMemory()
					+ "</li>");
			if (!isLoadIntoMemory()) {
				sbReturn.append("Temporary Directory: " + getTempDirectory()
						+ "</li>");
			}
		}
		sbReturn.append("</ul>");
		...
		if (isMultipartRequest()) {
			...
			if (fileEnumeration.hasMoreElements()) {
				...
				for (; fileEnumeration.hasMoreElements();) {
					...
					for (int i = 0; i < multiFiles.length; i++) {
						...
						sbReturn.append("\n<tr>"
								...
								+ "</td>"
								+ "<td>"
								+ (multiFile.getName() != null ? multiFile
										.getName() : "") + "</td>");

						sbReturn
								.append("<td>"
										+ (multiFile.getContentType() != null ? multiFile
												.getContentType()
												: "")
										+ "</td>"
										+ "<td>"
										+ (multiFile.getSize() != -1 ? multiFile
												.getSize()
												+ ""
												: "") + "</td>" + "</tr>");
					}
				...
				if (isMaxLengthExceeded()) {
					sbReturn
							.append("<p style=\"{color: red;}\">Max Bytes exceeded ("
									+ m_contentLength
									+ " > "
									+ m_maxContentLength
									+ ") all file uploads ignored.</p>");
				}
			}
		}
